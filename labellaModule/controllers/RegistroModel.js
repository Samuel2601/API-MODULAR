...
const registrarUsuario = async function (req, res) {
  try {
    ...
    if (!data.rol_user) {
      let rol_arr = await Model.Rol_user.findOne({ nombre: "Ciudadano" });
      if (!rol_arr) {
        let rol_arrc = await Model.Rol_user.create({
          nombre: "Ciudadano",
          orden: 4,
        });
        data.rol_user = rol_arrc._id;
      } else {
        data.rol_user = rol_arr._id;
      }
    }
    var admin = await Model.Usuario.findOne({ correo: data.correo });
    ...
    if (!admin) {
      ...
      res
        .status(409)
        .send({ message: "El correo ya existe en la base de datos" });
    }
  } catch (error) {
    console.error(error);
    res.status(500).send({ message: "Algo salió mal" });
...
const register = async function (data, ret) {
  try {
    ...
    const newUser = await Model.Usuario.create(data);
...
async function createDefaultRoleAndPermission(data) {
  let role = await Model.Rol_user.findOne({ nombre: "Ciudadano" });
  if (!role) {
    role = await Model.Rol_user.create({ nombre: "Ciudadano", orden: 4 });
  }
  data.role = role._id;
}
  ...
  if (req.user) {
    try {
      ...
      res
        .status(200)
        .send({
          message: "Actividad de proyecto registrada correctamente",
          data: nuevaActividad,
        });
    } catch (error) {
      res
        .status(500)
        .send({
          message: "Error al registrar la actividad de proyecto",
          error: error,
        });
    }
  } else {
    ...
    try {
      ...
      if (estado) {
        ...
        res
          .status(200)
          .send({
            message: "Incidente/denuncia registrado correctamente",
            data: nuevoIncidente,
          });
      } else {
        res
          .status(500)
          .send({
            message: "Error al registrar el incidente/denuncia",
            error: "No se han registrado Estado de Incidencia",
          });
      }
    } catch (error) {
      console.error(error);
  ...
  if (req.user) {
    try {
      ...
      if (find.length == 0) {
        ...
        res
          .status(500)
          .send({ message: "Ya registró la categoría", data: undefined });
      }
    } catch (error) {
      res
        .status(500)
        .send({ message: "Error al registrar la categoría", error: error });
    }
  } else {
    res.status(500).send({ message: "Acceso no permitido" });
  }
  ...
  if (req.user) {
    try {
      ...
      if (find.length == 0) {
        ...
        res
          .status(500)
          .send({ message: "Ya registró la subcategoría", data: undefined });
      }
    } catch (error) {
      res
        .status(500)
        .send({ message: "Error al registrar la subcategoría", error: error });
    }
  } else {
    res.status(500).send({ message: "Acceso no permitido" });
  }
...
const registrarEncargadoCategoria = async function (req, res) {
  if (req.user) {
    try {
      let data = req.body;
      let encargadoCategoriaExistente = await Model.Encargado_categoria.findOne(
      ...
      if (encargadoCategoriaExistente) {
        ...
        res
          ...
          .send({
            ...
            data: encargadoCategoriaExistente,
          });
      } else {
        ...
        res
          .status(200)
          .send({
            message: "Encargado de categoría registrado correctamente",
            data: nuevoEncargado,
          });
      }
    } catch (error) {
      res
        .status(500)
        .send({
          message: "Error al registrar o actualizar el encargado de categoría",
          error: error,
        });
    }
  } else {
...
const registrarRolUsuario = async function (req, res) {
  if (req.user) {
    try {
      ...
      const rolExistente = await Model.Rol_user.findOne({
        orden: req.body.orden,
      });
      if (rolExistente) {
        return res
          .status(400)
          .send({ message: "Ya existe un rol de usuario con el mismo orden" });
      }
...
const registrarPermiso = async function (req, res) {
  if (req.user) {
    try {
      let data = req.body;
      let permisoExistente = await Model.Permiso.findOne({
      ...
      res
        .status(500)
        .send({ message: "Error al registrar el permiso", error: error });
    }
  } else {
    res.status(500).send({ message: "Acceso no permitido" });
  }
  ...
  if (req.user) {
    try {
      ...
      res
        .status(200)
        .send({
          message: "Estado de incidente registrado correctamente",
          data: nuevoEstado,
        });
    } catch (error) {
      res
        .status(500)
        .send({
          message: "Error al registrar el estado de incidente",
          error: error,
        });
    }
  } else {
  ...
  if (req.user) {
    try {
      ...
      res
        .status(200)
        .send({
          message: "Estado de actividad de proyecto registrado correctamente",
          data: nuevoEstado,
        });
    } catch (error) {
      res
        .status(500)
        .send({
          message: "Error al registrar el estado de actividad de proyecto",
          error: error,
        });
    }
  } else {
  ...
  if (req.user) {
    try {
      ...
      res
        .status(200)
        .send({
          message: "Tipo de actividad de proyecto registrado correctamente",
          data: nuevoTipo,
        });
    } catch (error) {
      res
        .status(500)
        .send({
          message: "Error al registrar el tipo de actividad de proyecto",
          error: error,
        });
    }
  } else {
