...
const listarUsuarios = async function (req, res) {
  if (req.user) {
      ...
      if (campo && valor) {
        ...
        if (modelo == "rol_user" && campoRelacionado) {
          ...
          if (rolUser) {
            ...
            res
              .status(401)
              .send({ message: "Error al obtener el rol de usuario" });
          }
        } else {
          filtroConsulta[campo] = valor;
        }
      }
      var usuarios = await Model.Usuario.find(filtroConsulta)
        .sort({ createdAt: -1 })
        .populate("rol_user");
    ...
    res
      .status(500)
      .send({ message: "No se ha proporcionado un usuario válido" });
  }
};
...
const listarActividadesProyecto = async function (req, res) {
  if (req.user) {
    try {
      ...
      res.status(500).send({
        message: "Error al obtener la lista de actividades de proyecto",
        error: error,
      });
    }
  } else {
...
const listarIncidentesDenuncias = async function (req, res) {
  if (req.user) {
    try {
      ...
      if (all) {
        var incidentesDenuncias = await Model.Incidentes_denuncia.find(
          filtroConsulta
        )
          .sort({ createdAt: -1 })
          .populate("categoria")
      ...
      res.status(500).send({
        message: "Error al obtener la lista de incidentes/denuncias",
        error: error,
      });
    }
  } else {
...
const listarCategorias = async function (req, res) {
  if (req.user) {
    try {
      ...
      var categorias = await Model.Categoria.find(filtroConsulta).sort({
        createdAt: -1,
      });
...
const listarSubcategorias = async function (req, res) {
  if (req.user) {
    try {
      ...
      var subcategorias = await Model.Subcategoria.find(filtroConsulta)
        .sort({ createdAt: -1 })
        .populate("categoria");
      res.status(200).send({ data: subcategorias });
    } catch (error) {
      res.status(500).send({
        message: "Error al obtener la lista de subcategorías",
        error: error,
      });
    }
  } else {
...
const listarEncargadosCategorias = async function (req, res) {
  if (req.user) {
    try {
      ...
      var encargadosCategorias = await Model.Encargado_categoria.find(
        filtroConsulta
      )
        .sort({ createdAt: -1 })
        .populate("encargado")
      ...
      res.status(500).send({
        message: "Error al obtener la lista de encargados de categorías",
        error: error,
      });
    }
  } else {
...
const listarRolesUsuarios = async function (req, res) {
  if (req.user) {
    try {
      ...
      var rolesUsuarios = await Model.Rol_user.find(filtroConsulta).sort({
        orden: 1,
      });
      res.status(200).send({ data: rolesUsuarios });
    } catch (error) {
      res.status(500).send({
        message: "Error al obtener la lista de roles de usuarios",
        error: error,
      });
    }
  } else {
...
const listarPermisos = async function (req, res) {
  if (req.user) {
    try {
      ...
      componentes.forEach(async (componente) => {
        try {
          const nuevoPermiso = new Model.Permiso({
            ...
            rolesPermitidos: [
              "65d89dac488c56a8dc0114ce",
              "65c3e6710644fb2174bc6369",
            ],
          });
          const verf = await Model.Permiso.find({
            nombreComponente: componente,
          });
          if (verf.length == 0) {
          ...
          console.error(
            `Error al crear permiso para ${componente}: ${error.message}`
          );
        }
      });
      ...
      var permisos = await Model.Permiso.find(filtroConsulta)
        .sort({ createdAt: -1 })
        .populate("rolesPermitidos");
...
const listarEstadosIncidentes = async function (req, res) {
  if (req.user) {
    try {
      ...
      var estadosIncidentes = await Model.Estado_incidente.find(
        filtroConsulta
      ).sort({ createdAt: -1 });
      res.status(200).send({ data: estadosIncidentes });
    } catch (error) {
      res.status(500).send({
        message: "Error al obtener la lista de estados de incidentes",
        error: error,
      });
    }
  } else {
...
const listarEstadosActividadesProyecto = async function (req, res) {
  if (req.user) {
    try {
      ...
      var estadosActividadesProyecto =
        await Model.Estado_actividad_proyecto.find(filtroConsulta).sort({
          createdAt: -1,
        });
      ...
      res.status(500).send({
        message:
          "Error al obtener la lista de estados de actividad de proyecto",
        error: error,
      });
    }
  } else {
...
const listarTiposActividadesProyecto = async function (req, res) {
  if (req.user) {
    try {
      ...
      var tiposActividadesProyecto = await Model.Actividad_proyecto.find(
        filtroConsulta
      ).sort({ createdAt: -1 });
      ...
      res.status(500).send({
        message:
          "Error al obtener la lista de tipos de actividades de proyecto",
        error: error,
      });
    }
  } else {
...
const listarDireccionesGeo = async function (req, res) {
  if (req.user) {
    try {
      ...
      var direccionesGeo = await Model.Direccion_geo.find(filtroConsulta).sort({
        createdAt: -1,
      });
      res.status(200).send({ data: direccionesGeo });
    } catch (error) {
      res.status(500).send({
        message: "Error al obtener la lista de direcciones geográficas",
        error: error,
      });
    }
  } else {
