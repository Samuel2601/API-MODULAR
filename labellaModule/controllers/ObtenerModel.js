...
const obtenerUsuario = async function (req, res) {
  if (req.user) {
    ...
    try {
      let usuario = await Model.Usuario.findById({ _id: id }).populate(
        "rol_user"
      );
  ...
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({
          message: "Error al obtener la actividad de proyecto",
          error: error,
        });
    }
  } else {
  ...
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({
          message: "Error al obtener el incidente/denuncia",
          error: error,
        });
    }
  } else {
...
const obtenerCategoria = async function (req, res) {
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({ message: "Error al obtener la categoría", error: error });
    }
  } else {
    res.status(500).send({ message: "Acceso no permitido" });
  }
...
const obtenerSubcategoria = async function (req, res) {
  if (req.user) {
    var id = req.params["id"];
    try {
      let subcategoria = await Model.Subcategoria.findById({
        _id: id,
      }).populate("categoria");
      ...
      res
        .status(500)
        .send({ message: "Error al obtener la subcategoría", error: error });
    }
  } else {
    res.status(500).send({ message: "Acceso no permitido" });
  }
  ...
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({
          message: "Error al obtener el encargado de categoría",
          error: error,
        });
    }
  } else {
...
const obtenerRolUsuario = async function (req, res) {
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({ message: "Error al obtener el rol de usuario", error: error });
    }
  } else {
    res.status(500).send({ message: "Acceso no permitido" });
  }
  ...
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({
          message: "Error al obtener el estado de incidente",
          error: error,
        });
    }
  } else {
  ...
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({
          message: "Error al obtener el estado de actividad de proyecto",
          error: error,
        });
    }
  } else {
  ...
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({
          message: "Error al obtener el tipo de actividad de proyecto",
          error: error,
        });
    }
  } else {
  ...
  if (req.user) {
    ...
    try {
      ...
      res
        .status(500)
        .send({
          message: "Error al obtener la dirección geográfica",
          error: error,
        });
    }
  } else {
...
const verificarPermiso = async function (req, res) {
  if (req.user) {
    try {
      ...
      const permisoComponente = await Model.Permiso.findOne({
...
const obtenerPermisosRol = async function (req, res) {
  if (req.user) {
    try {
      ...
      const permisoComponente = await Model.Permiso.find({
        rolesPermitidos: rol_usuario,
      });
      const permisos = {};
